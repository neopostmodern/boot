...
class Microphone:
    def __init__(self):
        ...
        self._samplerate = sd.query_devices(self._device_index, "input")[
            "default_samplerate"
        ]
        self._stopped = False
    ...
    def monitor(
        self,
        timeout,
        callback_on_sound=lambda: None,
        callback_after_timeout=lambda: None,
        continuos=False,
        verbose=False,
        invert=False,
    ):
        ...
        def callback(indata, frames, time, status):
            ...
            if any(indata):
                ...
                if verbose:
                    ...
                    print(
                        f"→{loudness:.5f} {int(loudness * 20000) * c}"
                    )  # , magnitude)
            else:
                print("no input")
