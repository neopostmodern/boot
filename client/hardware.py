...
from .constants import OnboardPin, LedExtensionPin, ServoConfigs, ServoName, ServoSide, I2CAddress, Language
...
class LEDExtension():
  ...
  def lightshow(self, time_per_step = 1, reverse=False):
    pins = [LedExtensionPin.B_FLAT_MAJOR, LedExtensionPin.F_MAJOR, LedExtensionPin.C_MAJOR, LedExtensionPin.G_MINOR, LedExtensionPin.D_MAJOR, LedExtensionPin.D_MINOR, LedExtensionPin.A_MAJOR, LedExtensionPin.A_MINOR]
    if reverse:
      pins.reverse()
...
class StartButtons():
  ...
  def subscribe(self, language, callback):
    if language == Language.GERMAN:
      GPIO.add_event_detect(OnboardPin.START_BUTTON_DE_SWITCH, GPIO.FALLING, bouncetime=1000, callback=callback)
    else:
      raise ValueError("Unknown language: " + language)
...
class Servos:
  ...
  def set(self, servo_name: str, angle: float):
    ...
    if angle < servo_config.min_angle or angle > servo_config.max_angle:
      raise ValueError(f'Angle {angle:.1f}° outside of safe range [{servo_config.min_angle:.1f}°, {servo_config.max_angle:.1f}°] for servo "{servo_name.value}"!')
      
    if not self._psu.is_on():
      raise RuntimeError('PSU is not switched on')
    
    servo_driver = self._servos_left if servo_config.side == ServoSide.LEFT else self._servos_right
    servo_driver.set_pwm(servo_config.pin.value, 0, Servos._angle_to_pulse_width(angle))
    
class Drums:
  def __init__(self, servos: Servos):
    self._servos = servos
    
  def hit(self, servo_name: str, resting_angle: float, hit_angle: float, travel_time=None):     
    self._servos.set(servo_name, hit_angle)
    sleep(Servos.estimate_travel_time(servo_name, hit_angle - resting_angle) if travel_time is None else travel_time)
...
class Hardware:
  def __init__(self):
    ...
    #def restart_handler(channel):
    #  print("hey")
    #  self.start_buttons.turn_LED_on(Language.ENGLISH)      
    #  for i in range(50):
    #    print(i)
    #    if not self.start_buttons.probe(Language.ENGLISH):
    ...
    # 
    #print("ho")
    #GPIO.add_event_detect(OnboardPin.START_BUTTON_EN_SWITCH, GPIO.FALLING, bouncetime=1000, callback=restart_handler)
