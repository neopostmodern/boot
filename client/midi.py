...
def system_music_files(language):
    return [
        os.path.join(
            RENDER_BASE_PATH,
            config["client"]["music_directory_name"],
            f"song_{part}_{language}.wav",
        )
        if part is not None
        else None
        for part in ["a", None, "b", "c", "d"]
    ]
...
class MIDI:
        ...
        with mido.open_input() as in_port:
            for midi_message in in_port:
                ...
                if midi_message.channel == MidiChannel.DRUMS:
                    ...
                    print(
                        f"SYSTEM {midi_message.note} → {midi_message.note - 60} → {music_files[system_index]}"
                    )
    ...
    def play(midi_file_basename="master"):
        run(
            f"aplaymidi {config['client']['midi_directory_absolute_path']}/{midi_file_basename}.mid -p `aplaymidi -l | grep RtMidi | awk '{{print $1}}'` &",
            shell=True,
        )
