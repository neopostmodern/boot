...
class MCP23017(object):
    ...
    def _readAndChangePin(self, register, pin, value, curValue=None):
        assert pin >= 0 and pin < 8, "Pin number %s is invalid, only 0-%s are valid" % (
            pin,
            7,
        )
    ...
    def configPinInterrupt(self, pin, enabled, compareMode=0, defval=0):
        ...
        assert self.direction & (1 << pin) != 0, (
            "Pin %s not set to input! Must be set to input before you can change interrupt config."
            % pin
        )
    ...
    def readInterrupt(self, port=None):
        assert (
            self.mirrorEnabled == 1 or port != None
    ...
    def clearInterrupts(self):
        if self.i2c.readU8(MCP23017_INTFA) > 0 or self.i2c.readU8(MCP23017_INTFB) > 0:
            ...
            while count <= iterations:
                if (
                    self.i2c.readU8(MCP23017_INTFA) == 0
                    and self.i2c.readU8(MCP23017_INTFB) == 0
                ):
                    return 0
                else:
                    time.sleep(0.5)
