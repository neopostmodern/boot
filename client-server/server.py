        ...
        for nested_path in dir(prop):
            ...
            for parameter_name in sig.parameters:
                ...
                arguments.append(
                    {
                        "name": parameter_name,
                        "type": None if argument_type == "_empty" else argument_type,
                    }
                )

            system[path][nested_path] = {
                "arguments": arguments,
                "hints": documentation_hints,
            }
...
def constants_():
    ...
    for constant_name in dir(constants):
        if constant_name.startswith("_") or constant_name in [
            "Enum",
            "NamedTuple",
            "Union",
            "ServoConfig",
        ...
        if type(constant) in [str, int]:
            ...
            for property_name in dir(constant):
                ...
                constants_summary[constant_name][property_name] = getattr(
